	...
	private void procesarMensajeServidor(Mensaje mensaje) {
		...
		SwingUtilities.invokeLater(() -> {
			...
			case Mensaje.ESTADO_JUEGO:
				if (!partidaIniciada && (contenido.contains("Jugadores") || contenido.contains("Palabra:")
				...
				if (panelJuego != null) {
					...
					if (contenido.toLowerCase().contains("ganó") || contenido.toLowerCase().contains("ganado")
